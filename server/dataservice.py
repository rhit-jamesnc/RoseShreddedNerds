...
class DataService:
    ...
    def create_user(self, first_name, last_name, username, password_hash, dob, weight, role):
        ...
        sql_role = role.strip().capitalize()
        sql_dob = dob if dob and str(dob).strip() != "" else None
        try:
            sql_weight = int(weight) if weight else 0
        except:
            sql_weight = 0
        ...
        with pyodbc.connect(connection_string_database_copy) as conn:
            ...
            try:
                cursor.execute(add_person_exec, params)
                ...
                if row:
                    ...
                    raise Exception("Database failed to return a Generated ID.")
    ...
    def add_workout_exercises(self, user_id, workout_id, items):
        ...
        for item in items:
            ...
            pr_updated = self._maybe_update_pr(user_id, exercise_id, row["weight_kg"], row["reps"])
            if pr_updated:
    ...
    def list_workouts(self, user_id, from_date=None, to_date=None):
        ...
        # Sort most recent first: by date, then created_at
    ...
    def big3_leaderboard(self):
        ...
        for personal_record in self.db.get("personal_records").values():
            if personal_record["exercise_id"] in needed:
                ...
                # Initialize total for this user if needed
                if user_id not in totals:
                    totals[user_id] = 0.0

                # Add this PR's 1RM to their Big-3 total
