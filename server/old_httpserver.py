import old_dataservice as old_dataservice
import os
import json
...
ds = old_dataservice.DataService("shredded-nerds.db")
...
def login():
    ...
    session["user_id"] = user["id"]
    session.permanent = True
    ds.update_user(user_id=user["id"], last_login_at=old_dataservice.now_iso())
...
@app.get("/api/health")
def health():
    return jsonify({"ok": True, "time": old_dataservice.now_iso()})
